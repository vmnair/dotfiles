...
### Note icon breaking `added_date` parsing (2026-02-13)
- **Problem**: `<leader>tz` on a todo with an existing note prompted to create a new note
- **Cause**: In `parse_todo_line()`, the 󰈙 icon appears after `added_date` in storage format. The `added_date` regex requires the date at end of string, so the trailing icon prevented the match. Empty `added_date` caused `generate_todo_id()` to use today's date, producing a wrong ID.
- **Fix**: Moved note icon detection before `added_date` extraction in `parse_todo_line()`.
