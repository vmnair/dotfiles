...
-- Async function to get highlights from Readwise API
-- using Plenary.job
...
-- Get highlights with smart caching (uses cache id valid, fetches if stale)
-- @param callback function: function(data, error)
-- @param force_refresh boolean: If true, bypass cache and fetch fresh data
-- (optional, default false)
function M.get_highlights(callback, force_refresh)
	-- default force_refresh to false id not provided
	force_refresh = force_refresh or false

	-- Fast path: Try to use the cache
	if not force_refresh and is_cache_valid("highlights") then
		local cached_data, cache_err = load_cached_data("highlights")

		-- If cache loads successfully, return it immediately
		if not cache_err then
			callback(cached_data, nil)
			return
		end

		-- If  cache load failed, fall through to API fetch below
	end

	-- Slow path: Fetch fresh data from API
