...
function M.format_todo_line(todo)
	...
	-- Added date suppressed for cleaner display in active todos
...
-- Filter past due todos using a custom scratch buffer
-- Opens a clean window showing only overdue todos
function M.filter_todos_by_past_due()
	...
	-- Create new buffer for filtered results
	local buf = vim.api.nvim_create_buf(false, true)
	vim.api.nvim_buf_set_option(buf, "buftype", "nofile")
	vim.api.nvim_buf_set_option(buf, "swapfile", false)
	vim.api.nvim_buf_set_option(buf, "modifiable", true)
	-- Prepare content
	local lines = { "Past Due Todos (" .. #todos .. " found)", string.rep("=", 30), "" }
	for i, todo in ipairs(todos) do
		table.insert(lines, string.format("%d. %s", i, todo.text))
	end
	table.insert(lines, "")
	...
	vim.cmd("split")
	vim.api.nvim_win_set_buf(0, buf)
	vim.api.nvim_buf_set_name(buf, "Past Due Todos")
	-- Setup key mappings
	vim.keymap.set("n", "<CR>", function()
		local line_nr = vim.api.nvim_win_get_cursor(0)[1]
		if line_nr > 3 then -- Skip header lines
			local todo_index = line_nr - 3
			if todos[todo_index] then
				vim.cmd("close")
				vim.cmd("edit " .. todos[todo_index].source_file)
				vim.api.nvim_win_set_cursor(0, { todos[todo_index].line_num, 0 })
			end
		end
	end, { buffer = buf, desc = "Jump to todo" })
	vim.keymap.set("n", "<Space>", function()
