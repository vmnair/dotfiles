return {
	{
		...
		config = function()
			...
			vim.g.copilot_chat_model = vim.g.copilot_chat_model or "gpt-4o"

			-- Function to update tmux status with current model
			local function update_tmux_status()
				local current_model = vim.g.copilot_chat_model or "gpt-4o"
				...
				providers = {
					ollama = {
						prepare_input = function(chat_input, opts)
							-- Use default Copilot provider's prepare_input method but disable tools
							local input =
								require("CopilotChat.config.providers").copilot.prepare_input(chat_input, opts)
							-- Remove tools/functions to prevent "user skipped this function call" errors
							input.tools = nil
							input.tool_choice = nil
							return input
						end,
		...
		keys = {
			...
			{ "<leader>cce", "<Esc>:CopilotChatExplain<CR>", desc = "Explain Code", mode = { "n", "v" } },
			{
				...
				function()
					...
					-- Check if current model is a local Ollama model
					local is_local = string.match(current_model, ":")
						or string.match(current_model, "llama")
						or string.match(current_model, "qwen")
						or string.match(current_model, "gpt%-oss")

					-- If not using local model, switch to default local model
					if not is_local then
						vim.g.copilot_chat_model = "gpt-oss:20b"
						local cmd = string.format(
							'tmux setenv -g copilot_model "%s" && tmux refresh-client -S',
							vim.g.copilot_chat_model
						)
						vim.fn.system(cmd)
						vim.notify("Switched to local model: gpt-oss:20b", vim.log.levels.INFO)
					end

					-- Open CopilotChat with #buffer prompt
					vim.cmd("CopilotChat #buffer ")
				end,
				desc = "Ask Local LLM about Buffer",
			},
		},
	},
}
